...
const FRAMES: Record<string, Frame> = {
  ai: {
    ...
    solution: [
      ...
      "Trust Link Partnerでは、業務の棚卸しから活用設計、ユースケースの実装、社内定着、運用改善までを一貫して伴走します。経営者一人でも回せる現実的な規模にこだわり、過剰な作り込みや新たな属人化を生まない形で、確実に成果を積み上げていきます。",
    ],
    serviceName: "AI導入支援",
  ...
  claude: {
    ...
    solution: [
      ...
      "Trust Link PartnerのClaude導入支援では、プラン選定や初期設定から、業務テンプレートの整備、安全な運用ルールづくり、社内定着までを一貫して支援します。技術に不慣れな方でも日常的に使える状態まで伴走します。",
    ],
  ...
  "claude-code": {
    ...
    solution: [
      ...
      "Trust Link PartnerのClaude Code導入支援では、OSに合わせたセットアップ、活用ユースケースの設計、チーム運用の整備、定着・改善までを一貫して伴走します。CLI未経験でも使い始められるようサポートします。",
    ],
  ...
  dx: {
    ...
    solution: [
      ...
      "Trust Link PartnerのDX推進支援では、現状分析から構想策定、業務のデジタル化、情報の一元化、内製化までを伴走します。導入後も社内で改善を続けられる、持続可能な体制づくりを重視しています。",
    ],
    serviceName: "DX推進支援",
  ...
  crm: {
    ...
    solution: [
      ...
      "Trust Link Partnerでは、CRMの選定・導入から保守運用、活用度向上までをワンストップで支援します。導入して終わりにせず、投資を確実に成果へつなげます。",
    ],
  ...
  succession: {
    ...
    solution: [
      ...
      "Trust Link Partnerは、パチンコ店運営企業をはじめとする業界の事業承継に強みを持ち、承継先のマッチングから引き継ぎまでを秘密厳守で伴走します。業界特有の規制・商習慣を踏まえた現実的な承継を支援します。",
    ],
    serviceName: "事業承継支援",
  ...
  "realestate-dx": {
    ...
    solution: [
      ...
      "Trust Link Partnerの不動産DX支援では、CRMの提供と問い合わせ管理の効率化を中心に、現場が無理なく使える形で導入し、成果につながる運用まで伴走します。",
    ],
    serviceName: "不動産DX支援",
  ...
  pachinko: {
    ...
    solution: [
      ...
      "Trust Link Partnerは、業界知識とテクノロジーを掛け合わせ、AI・DX支援、事業承継支援、業態変更支援を通じてパチンコ業界の変革に伴走します。現場に根ざした実践的な支援を行います。",
    ],
    serviceName: "AI・DX支援",
    serviceSlug: "ai-support",
  },
  ...
  return [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        `とはいえ、自社だけで最適な進め方を判断するのは簡単ではありません。Trust Link Partnerは、業界知識とテクノロジーを掛け合わせ、${f.serviceName}をはじめとする伴走型の支援で企業の変革を支えています。現状の課題整理から最適なご提案まで、現場に寄り添ってサポートします。`,
...
export const POSTS: BlogPost[] = TOPICS.map((t, i) => {
  ...
  return {
    ...
    published,
    updated: published,
    keywords: [cat.label, "Trust Link Partner", FRAMES[t.cat].serviceName],
